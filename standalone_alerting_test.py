#!/usr/bin/env python3
"""Standalone test for alerting functionality."""

import sys
import os
import importlib.util

# Load alerting module directly without package import
...
def test_error_rate_alert():
    alert = alerting.ErrorRateAlert(threshold=0.03, duration="3m")
...
def test_latency_alert():
    alert = alerting.LatencyAlert(threshold_seconds=1.5, percentile="99", duration="3m")
    rule = alert.to_alert_rule()
...
def test_alert_manager():
    ...
    manager.add_alert(alerting.ErrorRateAlert(threshold=0.05))
    manager.add_alert(alerting.LatencyAlert(threshold_seconds=2.0))
...
if __name__ == "__main__":
    print("Running standalone alerting tests...")
    test_alert_rule()
    test_error_rate_alert()
    test_latency_alert()
    test_alert_manager()
    test_prometheus_integration()
    print("✅ All standalone alerting tests passed!")