...
class InputSanitizer:
    ...
    def sanitize_string(self, input_str: str, max_length: int = 10000) -> str:
        ...
        if threats_found:
            ...
            raise ValueError(
                f"Input contains dangerous patterns: {len(threats_found)} threats detected"
            )
    ...
    def validate_json_payload(
        self, payload: str, max_depth: int = 10, max_keys: int = 1000
    ) -> Dict[str, Any]:
        """
...
class RateLimiter:
    ...
    def __init__(self, redis_client=None):
        ...
        self.local_buckets = {}
        self.ip_tracking = defaultdict(
            lambda: {"requests": deque(), "blocked_until": None}
        )
    ...
    def is_allowed(
        self, identifier: str, rule_name: str = "default", check_burst: bool = True
        ...
        with self.lock:
            ...
            allowed_main = self._check_sliding_window(
                identifier, rule["requests"], rule["window"]
            )
            allowed_burst = True
    ...
    def _cleanup_old_data(self, current_time: float) -> None:
        ...
        for identifier, data in list(self.ip_tracking.items()):
            ...
            if not data["requests"] and (
                not data["blocked_until"] or data["blocked_until"] < current_time
            ):
    ...
    def get_rate_limit_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            blocked_ips = len(
                [
                    data
                    for data in self.ip_tracking.values()
                    if data["blocked_until"] and current_time < data["blocked_until"]
                ]
            )

            total_requests = sum(
                len(data["requests"]) for data in self.ip_tracking.values()
            )
...
class ThreatDetector:
    ...
    def __init__(self):
        ...
        self.anomaly_baselines = defaultdict(
            lambda: {"mean": 0, "std": 0, "samples": []}
        )
        self.lock = threading.RLock()
    ...
    def analyze_request(self, request_data: Dict[str, Any]) -> List[SecurityEvent]:
        ...
        for signature in self.threat_signatures:
            ...
            if threat_detected:
                ...
                logger.warning(
                    f"Security threat detected: {signature.name} from {source_ip}"
                )
    ...
    def get_security_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_source_ips": dict(
                sorted(events_by_ip.items(), key=lambda x: x[1], reverse=True)[:10]
            ),
            "threat_signatures_active": len(
                [s for s in self.threat_signatures if s.enabled]
            ),
        }
...
class SecurityHardeningManager:
    ...
    def is_trusted_ip(self, ip_address: str) -> bool:
        ...
        try:
            ...
            return any(ip in ip_network(network) for network in self.trusted_networks)
    ...
    def validate_and_secure_request(
        ...
        try:
            ...
            if not is_trusted and self.security_config.get(
                "rate_limiting_enabled", True
            ):
