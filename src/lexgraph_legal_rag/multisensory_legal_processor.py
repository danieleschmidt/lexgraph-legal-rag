...
class MultiSensoryAnalysis:
    ...
    scent_profile: Optional[DocumentScentProfile] = None
    
    def get_signal_by_channel(self, channel: SensoryChannel) -> Optional[SensorySignal]:
...
class TextualSensoryProcessor:
    ...
    async def process(self, document_text: str, metadata: Dict[str, Any]) -> SensorySignal:
        ...
        try:
            ...
            features = {
                ...
                "avg_sentence_length": self._calculate_avg_sentence_length(document_text),
            ...
            confidence = min(1.0, features["word_count"] / 100.0)  # Higher confidence with more text
    ...
    def _calculate_legal_term_density(self, text: str) -> float:
        ...
        legal_terms = [
            ...
            "statute", "regulation", "compliance", "violation", "penalty",
            "jurisdiction", "court", "judge", "attorney", "counsel",
            "plaintiff", "defendant", "evidence", "testimony", "verdict"
        ]
    ...
    def _calculate_textual_strength(self, features: Dict[str, Any]) -> float:
        ...
        strength = (0.3 * word_score + 0.2 * complexity_score + 
                   0.2 * diversity_score + 0.3 * legal_score)
...
class VisualSensoryProcessor:
    ...
    async def process(self, document_text: str, metadata: Dict[str, Any]) -> SensorySignal:
        ...
        try:
            features = {
                "structure_complexity": self._analyze_structure_complexity(document_text),
                "formatting_indicators": self._detect_formatting_indicators(document_text),
                "section_organization": self._analyze_section_organization(document_text),
    ...
    def _analyze_structure_complexity(self, text: str) -> float:
        ...
        numbered_items = len([line for line in text.split('\n') if line.strip() and line.strip()[0].isdigit()])
        
        complexity_score = min(1.0, (headings * 0.1) + (subsections * 0.05) + (numbered_items * 0.02))
    ...
    def _detect_formatting_indicators(self, text: str) -> Dict[str, int]:
        ...
        return {
            ...
            "indentation_levels": len(set(len(line) - len(line.lstrip()) for line in text.split('\n'))),
    ...
    def _analyze_section_organization(self, text: str) -> float:
        ...
        section_count = sum(1 for line in lines if any(marker in line for marker in section_markers))
    ...
    def _analyze_list_structure(self, text: str) -> Dict[str, Any]:
        ...
        numbered_lists = len([line for line in lines if line.strip() and 
                            len(line.strip()) > 2 and line.strip()[:2].replace('.', '').isdigit()])
        bulleted_lists = len([line for line in lines if line.strip().startswith(('•', '-', '*'))])
        nested_lists = len([line for line in lines if line.startswith(('  ', '\t')) and 
                          any(line.strip().startswith(marker) for marker in ['•', '-', '*', '1.', 'a.'])])
    ...
    def _calculate_visual_strength(self, features: Dict[str, Any]) -> float:
        ...
        formatting_score = min(1.0, sum(features["formatting_indicators"].values()) / 20.0)
        organization_score = features["section_organization"]
        
        list_score = min(1.0, features["list_structure"]["total_list_items"] / 10.0)
        
        # Weighted combination
        strength = (0.4 * structure_score + 0.2 * formatting_score + 
                   0.3 * organization_score + 0.1 * list_score)
...
class TemporalSensoryProcessor:
    ...
    def _extract_temporal_references(self, text: str) -> List[Dict[str, Any]]:
        ...
        temporal_patterns = [
            ...
            (r'\b(effective|commencing|beginning|starting|ending|expiring|until|before|after)\s+\w+', 'temporal_marker'),
    ...
    def _analyze_chronological_order(self, text: str) -> float:
        ...
        chronological_score = increasing_count / (len(year_positions) - 1) if len(year_positions) > 1 else 0.5
    ...
    def _calculate_temporal_density(self, text: str) -> float:
        ...
        temporal_density = len(temporal_refs) / (word_count / 100)  # References per 100 words
    ...
    def _detect_recency_indicators(self, text: str) -> Dict[str, Any]:
        ...
        recency_keywords = [
            "recent", "current", "latest", "new", "updated", "modern",
            "contemporary", "today", "now", "present", "ongoing"
        ]
        
        current_year = datetime.now().year
        recency_count = sum(1 for keyword in recency_keywords if keyword.lower() in text.lower())
        ...
        if context["creation_date"]:
            try:
                if isinstance(context["creation_date"], str):
                    creation_date = datetime.fromisoformat(context["creation_date"].replace('Z', '+00:00'))
                else:
    ...
    def _calculate_temporal_strength(self, features: Dict[str, Any]) -> float:
        ...
        # Weighted combination
        strength = (0.3 * ref_count_score + 0.3 * chronological_score + 
                   0.2 * density_score + 0.2 * recency_score)
...
class MultiSensoryLegalProcessor:
    ...
    async def process_document(self, document_text: str, document_id: str, 
                             metadata: Optional[Dict[str, Any]] = None) -> MultiSensoryAnalysis:
        """Process document through all available sensory channels."""
        metadata = metadata or {}
        
        self.logger.info(f"Processing document {document_id} through multi-sensory pipeline")
        ...
        if self.enable_olfactory:
            olfactory_task = self.olfactory_engine.analyze_document(document_text, document_id, metadata)
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, SensorySignal):
                ...
                olfactory_signal = SensorySignal(
                    ...
                    confidence=float(np.mean([s.confidence for s in result.signals]) if result.signals else 0.0),
                    features={
        ...
        self.logger.info(f"Multi-sensory analysis completed for {document_id} with "
    ...
    def _determine_primary_channel(self, signals: List[SensorySignal]) -> SensoryChannel:
    ...
    def _calculate_analysis_confidence(self, signals: List[SensorySignal]) -> float:
        ...
        total_weighted_confidence = sum(signal.strength * signal.confidence for signal in signals)
...
_multisensory_processor: Optional[MultiSensoryLegalProcessor] = None

def get_multisensory_processor(enable_olfactory: bool = True) -> MultiSensoryLegalProcessor:
    """Get or create global multi-sensory processor instance."""
    global _multisensory_processor
    if _multisensory_processor is None:
        _multisensory_processor = MultiSensoryLegalProcessor(enable_olfactory=enable_olfactory)
    return _multisensory_processor


async def analyze_document_multisensory(document_text: str, document_id: str, 
                                      metadata: Optional[Dict[str, Any]] = None) -> MultiSensoryAnalysis:
