    ...
    def validate_query(self, query: str) -> ValidationResult:
        ...
        if not isinstance(query, str):
            return ValidationResult(
                is_valid=False, sanitized_input="", errors=["Query must be a string"]
            )
    ...
    def _check_security_patterns(self, text: str) -> list[str]:
        ...
        if self.security_level in [SecurityLevel.STRICT, SecurityLevel.PARANOID]:
            for pattern in self.COMMAND_INJECTION_PATTERNS:
                if re.search(pattern, text_lower, re.IGNORECASE):
                    issues.append(
                        "Potentially dangerous pattern detected: Command injection"
                    )
                    break
    ...
    def _validate_content(self, query: str) -> list[str]:
        ...
        for pattern in non_legal_indicators:
            if re.search(pattern, query, re.IGNORECASE):
                warnings.append(
                    "Query contains terms not typically associated with legal searches"
                )
                break
...
def validate_query_input(
    query: str, security_level: SecurityLevel = SecurityLevel.STRICT
) -> ValidationResult:
...
def validate_document_content(content: str, source_path: str = "") -> ValidationResult:
    ...
    if not isinstance(content, str):
        return ValidationResult(
            is_valid=False, sanitized_input="", errors=["Content must be a string"]
        )
    ...
    has_legal_content = any(
        re.search(pattern, content, re.IGNORECASE) for pattern in legal_indicators
    )
    ...
    return ValidationResult(
        is_valid=is_valid, sanitized_input=content, warnings=warnings, errors=errors
    )
...
class ParameterValidator:
    ...
    def validate_top_k(top_k: Any) -> ValidationResult:
        ...
        if top_k < 1:
            return ValidationResult(
                is_valid=False, sanitized_input=1, errors=["top_k must be at least 1"]
            )
    ...
    def validate_hops(hops: Any) -> ValidationResult:
        ...
        if hops < 1:
            return ValidationResult(
                is_valid=False, sanitized_input=1, errors=["hops must be at least 1"]
            )
    ...
    for param_name, result in results.items():
        if not result.is_valid:
            ...
            logger.info(
                ...
                extra={"parameter": param_name, "validation_warnings": result.warnings},
            )
        else:
