...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.legal_corpus_path = self._get_optional_env(
        ...
        self.require_https = (
            self._get_optional_env("REQUIRE_HTTPS", "false").lower() == "true"
        )
    ...
    def validate_startup(self) -> None:
        ...
        is_development_key = (
            self.api_key.lower() in development_keys
            or "test" in self.api_key.lower()
            or "valid" in self.api_key.lower()
        )
        ...
        if is_development_key:
            ...
            if len(self.api_key) < 8:
                ...
                raise ConfigurationError(
                    "API_KEY must be at least 16 characters long for production"
                )

            # Validate key complexity
            if not self._has_sufficient_entropy(self.api_key):
                logger.warning(
                    "API_KEY has low entropy - consider using a more complex key"
                )
