    ...
    async def get_health(self, detailed: bool = False) -> SystemHealth:
        ...
        for check in self.checks:
            try:
                ...
                check_results.append(ComponentHealth(
...
def add_health_endpoints(app: FastAPI):
    ...
    async def health_metrics():
        ...
        metrics = []
        metrics.append(f'health_status{{service="lexgraph"}} {1 if health.status == HealthStatus.HEALTHY else 0}')
        metrics.append(f'health_uptime_seconds{{service="lexgraph"}} {health.uptime_seconds}')
        
        for check in health.checks:
            status_value = 1 if check.status == HealthStatus.HEALTHY else 0
            metrics.append(f'health_component_status{{component="{check.name}"}} {status_value}')
            
            if check.response_time_ms:
                metrics.append(f'health_component_response_time_ms{{component="{check.name}"}} {check.response_time_ms}')
