...
from .auth import setup_test_key_manager
...
from .versioning import SUPPORTED_VERSIONS as VERSIONING_SUPPORTED_VERSIONS
from .versioning import VersionedResponse
from .versioning import VersionNegotiationMiddleware
from .versioning import create_version_info_endpoint
from .versioning import get_request_version
    ...
    try:
        ...
        if key_manager.get_active_key_count() > 0:
            ...
            if not key_manager.is_valid_key(x_api_key):
                logger.warning(
                    "invalid API key attempt via key manager",
                    extra={"provided": x_api_key[:8] + "..."},
                )
                raise HTTPException(
            ...
            if not api_key or x_api_key != api_key:
                logger.warning(
                    "invalid API key attempt via legacy",
                    extra={"provided": x_api_key[:8] + "..."},
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid API key"
        ...
        if not api_key or x_api_key != api_key:
            logger.warning(
                "invalid API key attempt via fallback",
                extra={"provided": x_api_key[:8] + "...", "error": str(e)},
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid API key"
            )
...
class PingResponse(BaseModel):
    ...
    timestamp: float | None = Field(None, description="Response timestamp")

    model_config = ConfigDict(
        json_schema_extra={
            "examples": [{"version": "v1", "ping": "pong", "timestamp": 1234567890.123}]
        }
    )
...
class KeyRotationRequest(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"examples": [{"new_primary_key": "new-secure-api-key-123"}]}
    )
...
class KeyRevocationRequest(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"examples": [{"api_key": "old-api-key-to-revoke"}]}
    )
...
def create_api(
    ...
    def register_routes(router: APIRouter) -> None:
        ...
        async def add(
            ...
            formatted_response = VersionedResponse.format_response(
            ...
            if current_version.version == "v2":
                ...
                return AddResponse(
                    result=formatted_response.get("data", {}).get("result", result)
                )
            else:
    ...
    def register_health_routes(app: FastAPI) -> None:
        ...
        async def health() -> HealthResponse:
            ...
            return HealthResponse(status="healthy", version=version, checks=checks)
    ...
    def register_admin_routes(router: APIRouter) -> None:
        ...
        @router.post(
            "/admin/rotate-keys", response_model=KeyManagementResponse, tags=["Admin"]
        )
        ...
        @router.post(
            "/admin/revoke-key", response_model=KeyManagementResponse, tags=["Admin"]
        )
        ...
        @router.get(
            "/admin/key-status", response_model=KeyManagementResponse, tags=["Admin"]
        )
        async def key_status() -> KeyManagementResponse:
            ...
            return KeyManagementResponse(
                message="Key status retrieved successfully", rotation_info=rotation_info
            )
    ...
    def custom_openapi():
        ...
        openapi_schema["tags"] = [
            ...
            {"name": "Utilities", "description": "Utility endpoints for testing"},
            {
                "name": "API Info",
