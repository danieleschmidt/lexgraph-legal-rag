...
class AdaptiveCache:
    ...
    def _adapt_cache_size(self) -> None:
        ...
        if current_hit_rate > self.hit_rate_threshold and self.current_size < self.max_size:
            # Good hit rate, can increase cache size
            self.current_size = min(self.max_size, int(self.current_size * 1.2))
            logger.debug(f"Increased cache size to {self.current_size} (hit rate: {current_hit_rate:.3f})")
        elif current_hit_rate < self.hit_rate_threshold * 0.8 and self.current_size > self.initial_size:
            # Poor hit rate, decrease cache size
            self.current_size = max(self.initial_size, int(self.current_size * 0.8))
            logger.debug(f"Decreased cache size to {self.current_size} (hit rate: {current_hit_rate:.3f})")
...
class ConnectionPool:
        ...
        async with self._lock:
            while len(self._pool) < self.min_size:
                try:
                    ...
                    logger.debug(f"Created new pooled connection, total: {len(self._pool)}")
    ...
    async def acquire(self) -> T:
        ...
        async with self._lock:
            ...
            while self._pool:
                conn = self._pool[0]
                if current_time - self._created_at.get(id(conn), 0) > self.max_idle_time:
                    expired_conn = self._pool.popleft()
                    self._created_at.pop(id(expired_conn), None)
                    logger.debug(f"Removed expired connection from pool")
                else:
                    break
    ...
    async def release(self, conn: T) -> None:
        ...
        async with self._lock:
            ...
            if len(self._pool) < self.max_size:
                self._pool.append(conn)
                logger.debug(f"Returned connection to pool, available: {len(self._pool)}")
            else:
                self._created_at.pop(conn_id, None)
                logger.debug(f"Discarded excess connection")
...
class PerformanceProfiler:
    ...
    def get_operation_stats(self, operation_name: str, minutes: int = 5) -> Dict[str, Any]:
        """Get statistics for a specific operation."""
        cutoff_time = time.time() - (minutes * 60)
        
        with self._lock:
            recent_metrics = [
                m for m in self.metrics.get(operation_name, [])
        ...
        return {
            ...
            "throughput_ops_per_sec": sum(m.throughput_ops_per_sec for m in recent_metrics) / len(recent_metrics),
