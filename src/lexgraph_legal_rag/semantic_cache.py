...
class SemanticSimilarityMatcher:
    ...
    def calculate_similarity(self, keywords1: set[str], keywords2: set[str]) -> float:
        ...
        weighted_union = sum(self.legal_term_weights.get(word, 1.0) for word in union)
...
class SemanticQueryCache:
    ...
    def __init__(
        ...
        self._keyword_index: dict[str, set[str]] = defaultdict(
            set
        )  # keyword -> query_hashes
        ...
        logger.info(
            f"Initialized semantic cache: max_size={max_size}, ttl={ttl_seconds}s, threshold={similarity_threshold}"
        )
    ...
    async def get(self, query: str) -> tuple[str, dict[str, Any]] | None:
            ...
            if similar_entries:
                ...
                if not entry.is_expired(self.ttl_seconds):
                    ...
                    self.stats["avg_similarity_score"] = (
                        current_avg * (total_semantic - 1) + similarity_score
                    ) / total_semantic
                    ...
                    logger.info(
                        f"Semantic cache hit (similarity: {similarity_score:.3f}) for query: {query[:50]}..."
                    )
                    return entry.response, metadata
                else:
                    # Remove expired entry
    ...
    async def put(
        ...
        with self._access_lock:
            ...
            logger.debug(
                f"Cached response for query: {query[:50]}... (keywords: {len(keywords)})"
            )
...
async def cached_query(
    query: str, response_func, **kwargs
    ...
    try:
        ...
        metadata = {"cache_hit_type": "miss", "response_time": response_time}
