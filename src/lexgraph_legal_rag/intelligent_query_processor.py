...
class LegalTermExpander:
    ...
    def _select_best_synonym(
        self, term: str, synonyms: list[str], context: str
    ) -> str | None:
...
class IntelligentQueryProcessor:
    ...
    async def process_query(
        ...
        enhanced_query, synonyms_added, context_expansions = (
            self.term_expander.expand_query(query)
        )

        # Apply conversation context if available
        if conversation_id and self.conversation_context.get(conversation_id):
            enhanced_query = self._apply_conversation_context(
                enhanced_query, conversation_id
            )
    ...
    def _extract_legal_terms(self, query: str) -> list[str]:
        ...
        for term in self.term_expander.legal_synonyms:
            if term in query_lower:
                legal_terms.append(term)

        # Check for domain terms
        for domain in self.term_expander.domain_expansions:
    ...
    def analyze_query_patterns(self) -> dict[str, Any]:
        ...
        return {
            ...
            "popular_terms": dict(
                sorted(term_frequency.items(), key=lambda x: x[1], reverse=True)[:10]
            ),
            "average_query_length": sum(len(q.split()) for q in self.query_history)
