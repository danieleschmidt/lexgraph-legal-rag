...
class InputValidator:
    ...
    def __init__(self):
        ...
        self.forbidden_terms = {
            'confidential_client_data', 'attorney_work_product', 
            'privileged_communication', 'sealed_document'
        }
    
    async def validate_query(self, query: str, source_ip: Optional[str] = None) -> Tuple[bool, List[str]]:
        ...
        legal_term_count = len(re.findall(r'\b(contract|liability|breach|statute|law|legal|court|judge)\b', query_lower))
        ...
        if pii_found:
            violations.extend([f"PII detected: {pii_type}" for _, pii_type in pii_found])
...
class IntelligentRateLimiter:
    ...
    def __init__(self):
        ...
        self.client_reputation: Dict[str, float] = defaultdict(lambda: 1.0)  # 1.0 = good, 0.0 = blocked
    ...
    def _get_client_id(self, source_ip: str, user_id: Optional[str] = None) -> str:
    ...
    async def check_rate_limit(self, source_ip: str, user_id: Optional[str] = None,
                              client_tier: str = 'default') -> Tuple[bool, Dict[str, Any]]:
    ...
    def _update_reputation(self, client_id: str, change: float) -> None:
        ...
        if abs(change) >= 0.1:
            logger.info(f"Client {client_id} reputation: {current:.2f} -> {new_reputation:.2f}")
    
    def record_successful_request(self, source_ip: str, user_id: Optional[str] = None) -> None:
        """Record successful request to improve reputation."""
        with self._lock:
            client_id = self._get_client_id(source_ip, user_id)
            self._update_reputation(client_id, 0.01)  # Small positive increment
    
    def record_security_violation(self, source_ip: str, violation_severity: float,
...
class AnomalyDetector:
    ...
    async def analyze_query(self, query: str, source_ip: str, 
        ...
        with self._lock:
            ...
            recent_queries = [t for t in behavior['query_times'] if current_time - t <= 300]  # 5 minutes
...
class SecurityManager:
    ...
    async def validate_request(self, query: str, source_ip: str, 
        ...
        try:
            # Input validation
            is_valid, violations = await self.input_validator.validate_query(query, source_ip)
            security_info['violations'] = violations
            
            if violations:
                max_threat = ThreatLevel.HIGH if any('malicious' in v.lower() or 'forbidden' in v.lower() for v in violations) else ThreatLevel.MEDIUM
                security_info['threat_level'] = max_threat
            
            # Rate limiting check
            rate_allowed, rate_info = await self.rate_limiter.check_rate_limit(source_ip, user_id, client_tier)
            ...
            if not rate_allowed:
                ...
                self._record_security_event(
                    ...
                    ThreatLevel.MEDIUM,
                    source_ip, user_id, query, blocked=True
                )
            
            # Anomaly detection
            anomalies = await self.anomaly_detector.analyze_query(query, source_ip, user_id)
            ...
            if anomalies:
                ...
                if max_severity > 0.8:
                    ...
                    security_info['threat_level'] = max(security_info['threat_level'], ThreatLevel.MEDIUM)
            
            # Make final decision
            allowed = True
            
            # Block on critical threats
            if self.block_on_critical and security_info['threat_level'] == ThreatLevel.CRITICAL:
                allowed = False
                security_info['blocked_reason'] = 'critical_threat_detected'
            
            # Block if input validation failed severely
            if not is_valid and any('malicious' in v.lower() or 'forbidden' in v.lower() for v in violations):
                allowed = False
            ...
            if violations or anomalies or not allowed:
                ...
                self._record_security_event(
                    event_type, security_info['threat_level'],
                    source_ip, user_id, query, blocked=not allowed
                )
            else:
    ...
    def _determine_event_type(self, violations: List[str], anomalies: List[Tuple[str, float]]) -> SecurityViolationType:
    ...
    def _record_security_event(self, event_type: SecurityViolationType, 
                              threat_level: ThreatLevel, source_ip: Optional[str],
        ...
        with self._lock:
            event = SecurityEvent(
                ...
                source_ip=source_ip,
                user_id=user_id,
                query=query[:200] + ('...' if len(query) > 200 else ''),  # Truncate for logs
                timestamp=time.time(),
                blocked=blocked
            )
            ...
            if source_ip:
                ...
                self.rate_limiter.record_security_violation(source_ip, severity_score, user_id)
...
async def validate_request(query: str, source_ip: str, user_id: Optional[str] = None,
                          client_tier: str = 'default') -> Tuple[bool, Dict[str, Any]]:
