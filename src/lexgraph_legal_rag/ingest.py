...
def ingest_documents(
    ...
    try:
        ...
        if not docs_path.exists():
            print(
                f"Error: Documents directory {docs_path} does not exist",
                file=sys.stderr,
            )
            return 1
...
Examples:
  ...
  lexgraph-ingest --docs ./docs --index docs.bin --chunk-size 256 --metrics-port 9090
    ...
    parser.add_argument(
        "--docs", required=True, help="Directory containing legal documents to index"
    )
    parser.add_argument(
        "--index", default="index.bin", help="Output path for the vector index file"
    )
    parser.add_argument(
        "--semantic",
        action="store_true",
    ...
    parser.add_argument(
        "--metrics-port", type=int, help="Port to expose Prometheus metrics server"
    )
    parser.add_argument(
        "--log-level",
    ...
    try:
        ...
        print(
            "Warning: Configuration validation failed, proceeding anyway",
            file=sys.stderr,
        )
