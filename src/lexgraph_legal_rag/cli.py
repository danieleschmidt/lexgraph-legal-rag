...
Examples:
  ...
  lexgraph query "liability clauses" --metrics-port 9090
    ...
    query_parser.add_argument("query", help="Legal query to analyze")
    query_parser.add_argument(
        "--index", default="index.bin", help="Path to the vector index file"
    )
    query_parser.add_argument(
        "--hops", type=int, default=3, help="Number of context reasoning hops"
    )
    query_parser.add_argument(
        "--top-k", type=int, default=5, help="Number of documents to retrieve"
    )
    query_parser.add_argument(
        "--citations", action="store_true", help="Include detailed citations in output"
    )
    query_parser.add_argument(
    ...
    ingest_parser.add_argument(
        "--docs", required=True, help="Path to directory containing legal documents"
    )
    ingest_parser.add_argument(
        "--index", default="index.bin", help="Output path for the vector index"
    )
    ingest_parser.add_argument(
        "--semantic", action="store_true", help="Enable semantic search indexing"
    )
    ingest_parser.add_argument(
        "--chunk-size", type=int, default=512, help="Text chunk size for indexing"
    )
    ...
    serve_parser.add_argument(
        "--reload", action="store_true", help="Enable auto-reload in development"
    )

    # Global options
    parser.add_argument(
        "--metrics-port", type=int, help="Port for Prometheus metrics server"
    )
    parser.add_argument(
        "--log-level",
...
async def handle_query_command(args) -> int:
    ...
    try:
        ...
        if Path(args.index).exists():
            ...
            print(
                f"Warning: Index {args.index} not found, proceeding without documents"
            )
        ...
        if args.citations:
            ...
            if args.format == "json":
                results = []
                async for chunk in reasoner.reason_with_citations(
                    args.query, hops=args.hops
                ):
...
def handle_ingest_command(args) -> int:
    ...
    try:
        ...
        if not docs_path.exists():
            print(
                f"Error: Documents directory {args.docs} does not exist",
                file=sys.stderr,
            )
            return 1
        ...
        pipeline.ingest_directory(
            docs_path, chunk_size=args.chunk_size, enable_semantic=args.semantic
        )
...
def handle_serve_command(args) -> int:
    ...
    try:
        ...
        print("Starting LexGraph Legal RAG API server...")
...
def main() -> int:
    ...
    try:
        ...
        print(
            "Warning: Configuration validation failed, some features may not work properly",
            file=sys.stderr,
        )
