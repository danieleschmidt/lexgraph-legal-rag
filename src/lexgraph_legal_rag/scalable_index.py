...
from .document_pipeline import VectorIndex
...
class IndexPool:
    ...
    def __init__(
        ...
        self.master_index_path: Path | None = None
        ...
        self.query_cache = AdaptiveCache(
            initial_size=1000, max_size=10000, ttl_seconds=1800  # 30 minutes
        )

    async def initialize(self, index_path: Path | None = None) -> None:
        ...
        for _i in range(self.min_instances):
    ...
    async def _create_instance(self) -> str:
        ...
        try:
            ...
            instance = ScalableIndexInstance(
                index_id=instance_id, index=index, status=IndexStatus.READY
            )
    ...
    def _select_best_instance(self) -> ScalableIndexInstance | None:
    ...
    async def batch_search(
        self, queries: list[str], top_k: int = 5, use_cache: bool = True
    ...
    async def _parallel_batch_search(
        self, indexed_queries: list[tuple[int, str]], top_k: int, use_cache: bool
        ...
        for i, query_batch in enumerate(query_batches):
            if query_batch and i < len(ready_instances):
                ...
                task = asyncio.create_task(
                    self._execute_batch_on_instance(
                        instance, query_batch, top_k, use_cache
                    )
                )
                tasks.append(task)
    ...
    def _distribute_queries(
        self, indexed_queries: list[tuple[int, str]], num_instances: int
    ...
    async def _check_and_scale(self) -> None:
        ...
        logger.debug(
            f"Pool metrics: {len(ready_instances)} instances, "
            f"avg response time: {avg_response_time:.1f}ms"
        )

        # Scale up if response time is too high
        if (
            avg_response_time > self.scale_up_threshold
            and len(self.instances) < self.max_instances
        ):

            logger.info(
                f"Scaling up: avg response time {avg_response_time:.1f}ms > "
        ...
        # Scale down if response time is very low and we have excess instances
            ...
            avg_response_time < self.scale_down_threshold
            and len(ready_instances) > self.min_instances
        ):
            ...
            logger.info(
                f"Scaling down: avg response time {avg_response_time:.1f}ms < "
            ...
            for instance in self.instances.values():
                if instance.status == IndexStatus.READY:
                    try:
                        ...
                        instance.status = IndexStatus.READY
                        logger.debug(
                            f"Updated {instance.index_id} with {len(new_documents)} documents"
                        )
                    except Exception as e:
...
_global_index_pool: IndexPool | None = None
