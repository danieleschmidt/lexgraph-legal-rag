...
from .advanced_resilience import get_resilience_manager
from .advanced_resilience import resilient
from .advanced_resilience import setup_default_fallbacks
from .advanced_security import get_security_manager
from .advanced_security import validate_request
from .auto_optimizer import get_auto_optimizer
from .auto_optimizer import get_optimal_parameter
from .auto_optimizer import record_performance
from .distributed_intelligence import get_distributed_processor
from .distributed_intelligence import process_query_at_scale
from .intelligent_query_processor import QueryIntent

# Import our intelligent systems
from .intelligent_query_processor import get_query_processor
from .multi_agent import CitationAgent
from .multi_agent import ClauseExplainerAgent

# Import base components
from .multi_agent import RetrieverAgent
from .multi_agent import SummarizerAgent
...
class IntelligentMultiAgentSystem:
    ...
    async def process_query_intelligent(
        ...
        try:
            ...
            security_valid, security_info = await validate_request(
                query, source_ip, user_id
            )
            if not security_valid:
                logger.warning(
                    f"Security validation failed: {security_info.get('blocked_reason')}"
                )
            ...
            # 2. Query Enhancement
            query_enhancement = await self.query_processor.process_query(
            ...
            get_optimal_parameter("timeout_seconds", 30)
            optimizations_applied.append("parameter_optimization")

            # 5. Process Query (Distributed or Local)
            if use_distributed and query_enhancement.intent not in [
                QueryIntent.SEARCH
            ]:  # Complex queries go distributed
                response = await self._process_distributed(enhanced_query, priority)
                optimizations_applied.append("distributed_processing")
            else:
                response = await self._process_local(
                    enhanced_query, query_enhancement.intent, top_k
                )
    ...
    async def stream_response_with_citations(
        self, query: str, source_ip: str = "127.0.0.1", **kwargs
    ) -> AsyncIterator[str]:
        ...
        if self.pipeline and result.security_validated:
            try:
                ...
                for doc in docs:
                    ref = (
                        doc.metadata.get("path", doc.id)
                        if hasattr(doc, "metadata")
                        else doc.id
                    )
                    snippet = (
                        doc.text[:100] + "..."
                        if hasattr(doc, "text") and len(doc.text) > 100
                        else str(doc)[:100]
                    )
    ...
    async def analyze_query_complexity(self, query: str) -> dict[str, Any]:
        ...
        return {
            ...
            "complexity_score": len(enhancement.legal_terms)
            + len(enhancement.synonyms_added),
        }
...
def get_intelligent_multi_agent_system(
    pipeline: Any = None,
...
async def process_legal_query(
    query: str, source_ip: str = "127.0.0.1", **kwargs
) -> IntelligentQueryResult:
