...
class QualityGateValidator:
    ...
    async def _run_gate(self, gate_name: str, gate_func) -> None:
        ...
        try:
            ...
            logger.info(f"Quality gate '{gate_name}': {result.status.value} - {result.message}")
    ...
    async def _check_code_format(self) -> QualityGateResult:
        ...
        try:
            ...
            return QualityGateResult(
                gate_name="code_format",
                status=QualityGateStatus.FAIL,
                message=f"Code format check failed: {str(e)}",
                details={"error": str(e)}
            )
    ...
    async def _check_type_checking(self) -> QualityGateResult:
        ...
        try:
            result = await self._run_command([
                "python3", "-m", "mypy", str(self.src_path)
            ], capture_output=True)
    ...
    async def _check_security(self) -> QualityGateResult:
        ...
        try:
            result = await self._run_command([
                "python3", "-m", "bandit", "-r", str(self.src_path), 
            ...
            try:
                ...
                low_issues = len([i for i in bandit_data.get("results", []) if i.get("issue_severity") == "LOW"])
                ...
                if high_issues > 0:
                    ...
                    message = f"Multiple medium severity security issues: {medium_issues}"
                else:
                    status = QualityGateStatus.PASS
    ...
    async def _check_test_coverage(self) -> QualityGateResult:
        ...
        try:
            ...
            result = await self._run_command([
                "python3", "-m", "pytest", str(self.tests_path), 
            ...
            try:
                ...
                return QualityGateResult(
                    ...
                    details={
                        ...
                        "files_covered": coverage_data.get("totals", {}).get("num_statements", 0),
                        "tests_run": len([f for f in self.tests_path.rglob("test_*.py")])
                    }
                )
    ...
    async def _check_performance(self) -> QualityGateResult:
        ...
        try:
            ...
            perf_test_files = list(self.tests_path.glob("**/test_*performance*.py")) + \
            ...
            if result.returncode == 0:
                try:
                    ...
                    if not safety_data:  # Empty list means no vulnerabilities
                        ...
                        high_vulns = len([v for v in safety_data if v.get("severity", "").lower() == "high"])
                        medium_vulns = len([v for v in safety_data if v.get("severity", "").lower() == "medium"])
                        
                        if high_vulns > 0:
                            status = QualityGateStatus.FAIL
                            message = f"High severity vulnerabilities found: {high_vulns}"
                        elif medium_vulns > 3:
                            status = QualityGateStatus.WARNING
                            message = f"Multiple medium severity vulnerabilities: {medium_vulns}"
                        else:
                            status = QualityGateStatus.WARNING
                            message = f"Some vulnerabilities found: {len(safety_data)} total"
    ...
    async def _check_container_security(self) -> QualityGateResult:
        ...
        try:
            ...
            return QualityGateResult(
                gate_name="container_security",
                status=QualityGateStatus.FAIL,
                message=f"Container security check failed: {str(e)}",
                details={"error": str(e)}
            )
    ...
    async def _run_command(self, cmd: List[str], capture_output: bool = True) -> subprocess.CompletedProcess:
    ...
    def _generate_summary(self) -> None:
        ...
        passed = len([r for r in self.results.values() if r.status == QualityGateStatus.PASS])
        failed = len([r for r in self.results.values() if r.status == QualityGateStatus.FAIL])
        warnings = len([r for r in self.results.values() if r.status == QualityGateStatus.WARNING])
        skipped = len([r for r in self.results.values() if r.status == QualityGateStatus.SKIP])
        
        logger.info(f"Quality Gates Summary: {passed} passed, {failed} failed, {warnings} warnings, {skipped} skipped")
        
        if failed > 0:
            failed_gates = [name for name, result in self.results.items() if result.status == QualityGateStatus.FAIL]
        ...
        if failed == 0 and warnings <= 2:
            ...
            logger.warning("⚠️ NEEDS ATTENTION: Some quality gates need improvement before production")
        else:
            logger.error("❌ NOT PRODUCTION READY: Critical quality gates failed")


async def run_quality_gates(project_root: Path = None) -> Dict[str, Any]:
