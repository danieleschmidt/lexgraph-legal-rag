...
class RetrieverAgent:
    ...
    async def run(self, query: str) -> str:
        ...
        if self.pipeline is None:
            logger.warning(
                "No pipeline configured for RetrieverAgent, returning stub response"
            )
        ...
        try:
            ...
            for doc, score in results:
                try:
                    ...
                    source = (
                        doc.metadata.get("path", doc.id) if doc.metadata else doc.id
                    )
            ...
            logger.debug(
                "Retrieved %d documents with total length %d",
                len(results),
                len(combined_text),
            )
    ...
    async def batch_run(self, queries: list[str]) -> list[str]:
        ...
        if self.pipeline is None:
            logger.warning(
                "No pipeline configured for RetrieverAgent, returning stub responses"
            )
        ...
        if hasattr(self.pipeline, "batch_search"):
            ...
            batch_results = [
                self.pipeline.search(query, top_k=self.top_k) for query in queries
            ]
        ...
        for i, (query, results) in enumerate(zip(queries, batch_results)):
            ...
            logger.debug(
                "Query %d: Retrieved %d documents with total length %d",
                i,
...
class SummarizerAgent:
    ...
    def _process_text(self, text: str) -> str:
        ...
        for line in lines:
            ...
            if line.startswith("[Source:") or any(
...
class ClauseExplainerAgent:
    ...
    async def run(self, summary: str) -> str:
        ...
        if any(term in summary.lower() for term in ["contract", "agreement", "terms"]):
            ...
            explanations.append(
                "This appears to be from a legal case or court decision."
            )
...
class CitationAgent:
    ...
    def _snippet(self, doc: LegalDocument, query: str) -> str:
        ...
        start = 0 if idx == -1 else max(idx - self.window, 0)
...
class RouterAgent:
    ...
    def needs_summary_only(self, query: str) -> bool:
        ...
        has_summary_keywords = any(
            keyword in lowered for keyword in self.summary_keywords
        )
        has_explain_keywords = any(
            keyword in lowered for keyword in self.explain_keywords
        )
...
class MultiAgentGraph:
    ...
    async def _run_recursive(self, query: str, depth: int) -> str:
        ...
        logger.debug(
            "Depth %d: routing decision '%s' for query: %s",
            depth,
            routing_decision,
            query,
        )
        ...
        if routing_decision == "explain" or (
            routing_decision == "default" and self.router.decide(query)
        ):
    ...
    async def run_with_citations(
        ...
        query: str,
        pipeline: Any,
        top_k: int = 3,  # pipeline: LegalDocumentPipeline
    ) -> AsyncIterator[str]:
    ...
    async def _run_with_batch_optimization(
        ...
        routing_decision = self.router.analyze_query_complexity(query)
        logger.debug(
            "Batch optimization: routing decision '%s' for query: %s",
            routing_decision,
            query,
        )
        ...
        if routing_decision == "search":
            ...
            if routing_decision == "summary":
                answer = summary
            elif routing_decision == "explain" or (
                routing_decision == "default" and self.router.decide(query)
            ):
        ...
        logger.debug(
            "Batch optimization completed: answer length %d, docs retrieved %d",
            len(answer),
            len(docs),
        )
        return answer, docs
