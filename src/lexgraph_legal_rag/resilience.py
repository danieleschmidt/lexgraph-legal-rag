    ...
    async def call(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        async with self._lock:
            ...
            if self._should_attempt_reset():
                ...
                logger.info(f"Circuit breaker '{self.config.name}' attempting reset (HALF_OPEN)")
...
class Bulkhead:
    ...
    async def execute(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        try:
            ...
            raise TimeoutError(f"Bulkhead timeout: no available slots after {self.config.timeout}s")
        
        try:
            self.active_count += 1
            logger.debug(f"Bulkhead executing (active: {self.active_count}/{self.config.max_concurrent})")
...
def resilient(
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        retry_config = RetryConfig(max_attempts=retry_attempts) if retry_attempts > 1 else None
