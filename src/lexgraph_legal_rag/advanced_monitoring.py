...
class MetricCollector:
    ...
    def __init__(self, retention_hours: int = 24):
        self.metrics = defaultdict(
            lambda: deque(maxlen=retention_hours * 3600)
        )  # 1 second resolution
        ...
        self.system_collectors = {
            ...
            "network_bytes_sent": lambda: (
                psutil.net_io_counters().bytes_sent
                if hasattr(psutil, "net_io_counters")
                else 0
            ),
            "network_bytes_recv": lambda: (
        ...
        with self.lock:
            for metric_name in self.metrics:
                stats = self.get_metric_stats(
                    metric_name, duration_seconds=300
                )  # Last 5 minutes
                if stats["count"] > 0:
...
class AnomalyDetector:
    ...
    def calculate_baseline(
        ...
        logger.info(
            f"Baseline calculated for {metric_name}: "
    ...
    def detect_anomaly(
        ...
        # Calculate z-score
        z_score = (
            abs(current_value - baseline.mean) / baseline.std_dev
...
class PredictiveAnalytics:
    ...
    def train_prediction_model(
        ...
        logger.info(
            f"Prediction model trained for {metric_name}: "
        ...
        return {
            ...
            "sample_count": len(values),
            "model_accuracy": (
                "good" if score > 0.7 else "fair" if score > 0.5 else "poor"
            ),
        }

    def predict_future_values(
        self, metric_name: str, future_minutes: int = 60
    ) -> Optional[Dict[str, Any]]:
    ...
    def detect_capacity_issues(
        self, metric_name: str, threshold_percent: float = 80.0
    ) -> Optional[Dict[str, Any]]:
        """Detect potential capacity issues based on predictions."""
        prediction = self.predict_future_values(
            metric_name, future_minutes=120
        )  # 2 hours ahead
                ...
                capacity_alerts.append(
                    {
                        ...
                        "severity": (
                            "high" if pred_point["predicted_value"] > 90 else "medium"
                        ),
                    }
                )
        ...
        if capacity_alerts:
            return {
                ...
                "earliest_alert_minutes": min(
                    alert["minutes_ahead"] for alert in capacity_alerts
                ),
                "detected_at": datetime.now().isoformat(),
            }
    ...
    def evaluate_alerts(self, metrics: Dict[str, Any]) -> List[Alert]:
        ...
        with self.lock:
            for rule in self.alert_rules:
                ...
                condition_met = False
                if (
                    (rule["condition"] == "gt" and current_value > rule["threshold"])
                    or (rule["condition"] == "lt" and current_value < rule["threshold"])
                    or (
                        rule["condition"] == "eq" and current_value == rule["threshold"]
                    )
                ):
                    condition_met = True
                ...
                if condition_met:
                    ...
                    alert = Alert(
                        ...
                        metadata={"current_value": current_value, "rule": rule.copy()},
                    )
...
class AdvancedMonitoringSystem:
    ...
    def _monitoring_loop(self) -> None:
        ...
        while self.monitoring_active:
            try:
                ...
                if (
                    current_time - last_baseline_calculation
                    > baseline_calculation_interval
                ):
                ...
                # Evaluate alerts
                self.alert_manager.evaluate_alerts(metrics_summary)
        ...
        for metric_name, stats in metrics_summary.items():
            if stats["count"] >= 10:  # Need minimum data points
                values = self.metric_collector.get_metric_values(
                    metric_name, duration_seconds=3600
                )
                try:
                    self.anomaly_detector.calculate_baseline(metric_name, values)
                except Exception as e:
                    logger.warning(
                        f"Failed to calculate baseline for {metric_name}: {e}"
                    )
    ...
    def _run_health_checks(self) -> None:
        ...
        for check_name, health_check in self.health_checks.items():
            ...
            if (
                ...
                or (current_time - health_check.last_check).total_seconds()
                ...
                try:
                    ...
                    if result.get("status") == "healthy":
                        ...
                        if (
                            health_check.current_successes
    ...
    def _check_capacity_issues(self) -> None:
        ...
        for metric_name in key_metrics:
            values = self.metric_collector.get_metric_values(
                metric_name, duration_seconds=7200
            )  # 2 hours
            timestamps = [
                time.time() - (len(values) - i - 1) * 5 for i in range(len(values))
            ]
            ...
            if len(values) >= 20:
                try:
                    ...
                    if capacity_alert:
                        logger.warning(
                            f"Capacity issue predicted for {metric_name}: "
    ...
    def get_monitoring_dashboard(self) -> Dict[str, Any]:
        ...
        if any(alert.severity == "critical" for alert in active_alerts):
            ...
            any(alert.severity == "warning" for alert in active_alerts)
            or anomaly_summary["anomalies_detected"] > 0
        ):
        ...
        return {
            ...
            "monitoring_status": {
                ...
                "uptime_seconds": (
                    time.time() - self.metric_collector.start_time
                    if hasattr(self.metric_collector, "start_time")
                    else 0
                ),
            },
        }
...
def main():
    ...
    try:
        ...
        for i in range(30):
            ...
            monitoring_system.metric_collector.increment_counter(
                "api_requests_total", 1
            )
            monitoring_system.metric_collector.increment_counter(
                "api_request_duration_sum", 0.5
            )
        ...
        print(
            f"❗ Anomalies Detected: {dashboard['anomaly_summary']['anomalies_detected']}"
        )
