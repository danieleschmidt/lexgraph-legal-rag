...
class APIKeyManager:
    ...
    def check_rate_limit(self, api_key: str, limit: int = 60, window: int = 60) -> bool:
        ...
        with self._lock:
            ...
            if len(key_requests) >= limit:
                logger.warning(
                    f"Rate limit exceeded for API key {self._hash_key(api_key)}"
                )
                return False
    ...
    def revoke_key(self, api_key: str) -> None:
        ...
        with self._lock:
            if api_key in self._active_keys:
                ...
                logger.warning(
                    f"Attempted to revoke unknown API key (hash: {self._hash_key(api_key)})"
                )

    def rotate_keys(self, new_primary_key: str) -> None:
        """Rotate API keys - add new primary and optionally revoke old ones."""
        with self._lock:
            self._active_keys.copy()
            self.add_key(new_primary_key)
            self._last_rotation = time.time()

            logger.info(
                f"Key rotation completed. Total active keys: {len(self._active_keys)}"
            )
        ...
        with self._lock:
            ...
            return {
                ...
                "average_key_age_days": (
                    sum(key_ages) / len(key_ages) if key_ages else 0
                ),
    ...
    def _hash_key(self, api_key: str) -> str:
        ...
        return hmac.new(self._secret_key, api_key.encode(), hashlib.sha256).hexdigest()[
            :8
        ]
