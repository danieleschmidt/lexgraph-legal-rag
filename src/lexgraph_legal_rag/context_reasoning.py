...
from .performance_optimization import performance_monitor, AdaptiveCache
from .validation import QueryValidator, SecurityLevel
from .resilience import ResilientOperation, RetryConfig, get_circuit_breaker, CircuitBreakerConfig
...
class ContextAwareReasoner:
    ...
    query_validator: QueryValidator = field(default_factory=lambda: QueryValidator(SecurityLevel.STRICT))
    ...
    async def reason(self, query: str, hops: int = 3) -> str:
        ...
        resilient_reasoning = ResilientOperation(
            circuit_breaker=get_circuit_breaker("context_reasoning", CircuitBreakerConfig(
                failure_threshold=5,
        ...
        try:
            ...
            return f"I apologize, but I encountered an error while analyzing your query about legal matters. Please try rephrasing your question or contact support if the issue persists."
    ...
    async def _execute_reasoning(self, query: str, hops: int) -> str:
        ...
        if self.use_scalable_index:
            ...
            await index_pool.initialize(self.pipeline.index._docs if hasattr(self.pipeline.index, '_docs') else None)
    ...
    def _build_optimized_context(
        ...
        for doc, score in sorted_results:
            ...
            context_entry = f"[Source: {source}, Relevance: {score:.3f}]\n{relevant_text}"
    ...
    def _extract_relevant_text(self, text: str, query: str, max_length: int = 800) -> str:
