    ...
    def record_metric(
        ...
        with self._lock:
            metric = PerformanceMetric(
                name=name, value=value, timestamp=time.time(), metadata=metadata or {}
            )
...
class AdaptiveOptimizer:
    ...
    async def record_query_performance(
        ...
        self.performance_monitor.record_metric(
            "error_rate", 0.0 if success else 1.0, metadata
        )
        ...
        if metadata:
            ...
            if "memory_usage_pct" in metadata:
                self.performance_monitor.record_metric(
                    "memory_usage", metadata["memory_usage_pct"] / 100
                )
    ...
    async def _maybe_optimize(self) -> None:
        ...
        response_time_data = self.performance_monitor.metrics.get(
            "response_time", deque()
        )
    ...
    async def optimize(self) -> list[OptimizationAction]:
        ...
        with self._lock:
            ...
            logger.info(
                f"Optimization cycle complete. Applied {len(actions)} optimizations"
            )
            return actions

    async def _address_performance_issue(
        self, issue: dict[str, Any], trends: dict[str, str]
        ...
        if metric == "response_time":
            ...
            if value > 3.0:  # Very slow responses
                new_ttl = max(
                    600, int(self.current_parameters["cache_ttl"] * (1 - change_rate))
                )
            ...
            if value < 0.5:  # Very low hit rate
                new_size = min(
                    5000, int(self.current_parameters["cache_size"] * (1 + change_rate))
                )
                new_threshold = max(
                    0.5, self.current_parameters["similarity_threshold"] - 0.1
                )
                ...
                return OptimizationAction(
                    ...
                    old_value={
                        "size": self.current_parameters["cache_size"],
                        "threshold": self.current_parameters["similarity_threshold"]
                        + 0.1,
                    },
            ...
            if value > 0.05:  # >5% error rate
                new_timeout = min(
                    60,
                    int(self.current_parameters["timeout_seconds"] * (1 + change_rate)),
                )
                ...
                return OptimizationAction(
                    ...
                    old_value={
                        "timeout": self.current_parameters["timeout_seconds"]
                        - int(self.current_parameters["timeout_seconds"] * change_rate),
    ...
    async def _proactive_optimizations(
        ...
        if (
            trends.get("response_time") == "improving"
            and trends.get("cache_hit_rate") == "stable"
        ):
    ...
    def get_optimization_report(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "success_rate": (
                    successful_optimizations / len(recent_actions)
                    if recent_actions
                    else 0
                ),
                ...
                "next_optimization_due": self.last_optimization
                + self.strategy_settings[self.strategy]["optimization_interval"],
            }
    ...
    async def evaluate_optimization_impact(
        self, action: OptimizationAction, duration_seconds: int = 300
    ) -> float:
...
async def record_performance(
    query: str, response_time: float, success: bool, **metadata
) -> None:
