...
class VersionNegotiationMiddleware(BaseHTTPMiddleware):
    ...
    def _negotiate_version(self, requested_version: str | None) -> APIVersion:
        """Negotiate the API version to use."""

        if not requested_version:
            logger.debug(
                "No version specified, using default: %s", self.default_version
            )
...
def version_deprecated(
    deprecated_in: str, removed_in: str, alternative: str | None = None
):
...
def create_version_info_endpoint():
    ...
    async def version_info():
        ...
        return {
            ...
            "deprecations": {
                "v1": {"deprecated_in": None, "removed_in": None, "status": "stable"}
            },
        }
